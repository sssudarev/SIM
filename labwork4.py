...
def students_criteria2(x, y, y_aver, n, m):
...
def students_criteria(x, y_average, n, m, dispersion):
...
def fisher_criteria(y, y_average, y_new, n, m, d, dispersion):
...
def check(X, Y, B, n, m, norm=False):
    ...
    ts = students_criteria2(X[:, 1:], Y, y_aver, n, m)
    ...
    Fp = fisher_criteria(Y, y_aver, y_new, n, m, d, dispersion_arr)
...
def with_interaction_effect(n, m):
    ...
    B_norm = find_coefficients(X_norm, y_aver, norm=True)
...
def linear(n, m):
    ...
    qq = (1 + 0.95) / 2
    student_cr_table = t.ppf(df=f3, q=qq)
    student_t = students_criteria(x_norm[:, 1:], y_average, n, m, dispersion_arr)
